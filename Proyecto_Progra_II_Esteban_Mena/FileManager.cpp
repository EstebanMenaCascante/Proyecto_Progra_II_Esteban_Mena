...
void FileManager::saveRoutesToFile(const RouteList& routeList, const std::string& filename) {
    ...
    while (currentRoute != nullptr) {
        outFile << "Route: " << currentRoute->name << std::endl;  // Añadir el prefijo "Route:"
...
void FileManager::loadRoutesFromFile(RouteList& routeList, const std::string& filename, sf::RenderWindow& window, sf::Font& font) {
    ...
    while (std::getline(inFile, line)) {
        ...
        if (line == "END_ROUTE") {
            routeName.clear();  // Marcar fin de la ruta actual
        }
        else if (line.rfind("Route: ", 0) == 0) {  // Nueva ruta
            routeName = line.substr(7);  // Quitar el prefijo "Route: "
            std::cout << "Ruta encontrada: " << routeName << std::endl;
            routeList.addRoute(routeName);
        }
        else if (line.rfind("Point: ", 0) == 0) {  // Leer punto
            std::istringstream iss(line.substr(7));  // Quitar "Point: "
            std::string pointName, xStr, yStr;

            // Extraer nombre, x, y usando coma como delimitador
